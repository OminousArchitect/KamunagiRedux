...
namespace KamunagiOfChains.Data.Bodies.Kamunagi.Passive
{
	...
	public class KamunagiDash : Asset, ISkill, IProjectile, IProjectileGhost
	{
		...
		{
			var proj = LoadAsset<GameObject>("RoR2/Base/Grandparent/GrandparentGravSphere.prefab")!.InstantiateClone("TwinsVacuumSphere", true);
		...
		GameObject IProjectileGhost.BuildObject()
		{
			...
			sparks.GetComponent<ParticleSystemRenderer>().material = LoadAsset<Material>("RoR2/Base/Common/VFX/matTracerBrightTransparent.mat");
			sparks.transform.localScale = Vector3.one * 0.25f;

			var ghost = PrefabAPI.InstantiateClone(LoadAsset<GameObject>("RoR2/Base/Blackhole/GravSphere.prefab").transform.GetChild(2).gameObject, "TwinsVacuumGhost", false);
	...
	public class RequiredTetherVFX : Asset, IEffect
	{
		...
		{
			var mandatory = LoadAsset<GameObject>("RoR2/Base/Grandparent/GrandparentGravSphereTether.prefab")!.InstantiateClone("ImTooLazyToDoThis", false);
	...
	public class FlyEffect : Asset, IEffect
	{
		public GameObject BuildObject()
		{
			var effect = LoadAsset<GameObject>("RoR2/Base/Mage/MuzzleflashMageLightningLargeWithTrail.prefab")!.InstantiateClone("TwinsFlyUpEffect", false);
			...
			trailRenderer.materials[0].SetTexture("_RemapTex", LoadAsset<Texture2D>("RoR2/Base/Common/ColorRamps/texRampAncientWisp.png"));
			return effect;
		}
	}
}